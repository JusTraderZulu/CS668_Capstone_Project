...
# Import our modules
from utils.data_processing import load_data, split_data, add_indicators, TrainingLogger, visualize_trade_actions, set_seed
from envs.trading_env import ForexTradingEnv, TraderConfig
from agents.dql_agent import DQLAgent
from utils.metrics import calculate_trading_metrics, create_performance_dashboard, create_training_plots
...
def train_agent(args, experiment_name: str):
    ...
    data_path = os.path.join("data", args.data_file)
    ...
    logger.log_system_info(train_env, agent)
    ...
    if args.test:
        ...
        from evaluation.vectorbt_eval import evaluate_with_vectorbt
        ...
        # Only try to log this if the file exists
        if os.path.exists("results/equity_curve.png"):
            mlflow.log_artifact("results/equity_curve.png", "test_artifacts")
        
        # Log all test artifacts to a test folder
        mlflow.log_artifact(dashboard_path, "test_artifacts")
        mlflow.log_artifact(viz_path, "test_artifacts")
        mlflow.log_artifact(test_metrics_path, "test_artifacts")
